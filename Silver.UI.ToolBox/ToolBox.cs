...
namespace Silver.UI
{
    ...
    public class ToolBox : UserControl
    {
        ...
        {
            ToolBox.DrawBorders(g,rect,bDepressed,Color.Empty);
        }
        ...
        private ToolBoxTabCollection _toolBoxTabs;
        ...
        [NonSerialized]
        private ImageAttributes      _dImgAttr;
        ...
        private void Initialize()
        {
            ...
            _textBox.KeyDown        += this.OnTextBox_KeyDown;
            _textBox.LostFocus      += this.OnTextBox_LostFocus;
        ...
        */
            ...
            get
            {
                if(null == _dImgAttr)
                {
                    ...
                    float[][]   matrix = {
                            ...
                            new[] {0.1f ,0.1f ,0.1f ,1, 1},
                            new[] {0.1f ,0.1f ,0.1f ,1, 1},
                            new[] {0.3f ,0.3f ,0.3f ,1, 1},
                            new[] {0.08f,0.08f,0.08f,0, 1},
                            new float[] {1    ,1    ,1    ,1, 1}
                    };
            ...
            try
            {
                if(null != _patBltItem || null != item)
                {
                    ...
                    if(null != _patBltItem)
                    {
                        ...
                        ToolBox.PatBlt(hdc,rct.X+1,rct.Y+1,rct.Width-2,rct.Height-2,ToolBox.DSTINVERT);
                    }
        ...
        internal void OnTabSelectionChanged(ToolBoxTab tab, LayoutFinished layoutFinished)
        {
            ...
            if(!_isLoading)
            {
                ...
                if(oldTab != tab)
                {
                    ...
                    {
                        _timer            = new Timer();
                        _timer.Tick      += this.OnTimer_LayoutElapsed;
        ...
        public bool SetImageList(Image image, Size size, Color transparentColor, bool small)
        {
            ...
            if(small)
            {
                ...
                if(null != image)
                {
                    _smallImageList = ToolBox.CreateImageListFromImage(image,size,transparentColor);
                }
            ...
            else
            {
                ...
                if(null != image)
                {
                    _largeImageList = ToolBox.CreateImageListFromImage(image,size,transparentColor);
                }
        ...
        private void CreateScrollTimer(ScrollDirection dir)
        {
            ...
            {
                _timer.Tick +=this.OnTimer_UpScrollElapsed;
            }
            else
            {
                _timer.Tick +=this.OnTimer_DnScrollElapsed;
            }
            _timer.Enabled = true;
        }
        ...
        private bool DoTimedLayout(ToolBoxTab oldSelectedTab)
        {
            ...
            {
                Debug.WriteLine("Control Location " + _selectedTab.Control.Location + " Item Area " + _selectedTab.ItemArea);
            }
        ...
        private void DoLayout(bool bInitial, bool bUpdateItems, bool bRepaint)
        {
            ...
            Debug.WriteLine("ToolBox DoLayout called...");
        ...
        private void HandleToolBoxItemDrop(DragEventArgs e)
        {
            ...
            try
            {
                ...
                _selectedTab.SwapItems(dragItem,this._dropItem);
            }
            catch
            {
            }
        ...
        private XmlElement SerializeToolBoxItem(ToolBoxItem item, XmlDocument xmlDoc, string nodeName)
        {
            ...
            // Caption
            element2 = xmlDoc.CreateElement(ToolBox.Xml_Caption_Node);
            ...
            // Image index
            element2 = xmlDoc.CreateElement(ToolBox.Xml_ImageIdxs_Node);
            element2.SetAttribute(ToolBox.Xml_SmallImgIdx_Attr,item.SmallImageIndex.ToString());
            element2.SetAttribute(ToolBox.Xml_LargeImgIdx_Attr,item.SmallImageIndex.ToString());
            itemElement.AppendChild(element2);

            // Tool tip
            element2 = xmlDoc.CreateElement(ToolBox.Xml_ToolTip_Node);
            ...
            // Enabled
            itemElement.SetAttribute(ToolBox.Xml_Enabled_Attr,item.Enabled.ToString().ToLower());

            // Movable
            itemElement.SetAttribute(ToolBox.Xml_Movable_Attr,item.Movable.ToString().ToLower());

            // Renamable
            itemElement.SetAttribute(ToolBox.Xml_Renamable_Attr,item.Renamable.ToString().ToLower());

            // Deletable
            itemElement.SetAttribute(ToolBox.Xml_Deletable_Attr,item.Deletable.ToString().ToLower());

            // Allow drag
            itemElement.SetAttribute(ToolBox.Xml_AllowDrag_Attr,item.AllowDrag.ToString().ToLower());
        ...
        private void DeSerializeToolBoxItem(ToolBoxItem item, XmlElement itemNode)
        {
            ...
            // Caption
            element2          = (XmlElement)itemNode.SelectSingleNode(ToolBox.Xml_Caption_Node);
            ...
            // Tool tip
            element2          = (XmlElement)itemNode.SelectSingleNode(ToolBox.Xml_ToolTip_Node);
            ...
            // Image indices
            element2             = (XmlElement)itemNode.SelectSingleNode(ToolBox.Xml_ImageIdxs_Node);
            item.SmallImageIndex = SafeInt32FromString(element2.GetAttribute(ToolBox.Xml_SmallImgIdx_Attr)  ,-1);
            item.LargeImageIndex = SafeInt32FromString(element2.GetAttribute(ToolBox.Xml_LargeImgIdx_Attr)  ,-1);

            // Movable, Renamable, Deletable, Allow Drag
            item.Movable   = SafeBoolFromString(itemNode.GetAttribute(ToolBox.Xml_Movable_Attr)  ,true);
            item.Renamable = SafeBoolFromString(itemNode.GetAttribute(ToolBox.Xml_Renamable_Attr),true);
            item.Deletable = SafeBoolFromString(itemNode.GetAttribute(ToolBox.Xml_Deletable_Attr),true);
            item.AllowDrag = SafeBoolFromString(itemNode.GetAttribute(ToolBox.Xml_AllowDrag_Attr),true);
        ...
        protected override void OnKeyDown(KeyEventArgs e)
        {
            ...
            ctrlPressed = Keys.Control == (Control.ModifierKeys & Keys.Control);
            ...
            if(null != _selectedTab)
            {
                ...
                {
                    SwitchTabs((Keys.Shift == (Control.ModifierKeys & Keys.Shift)));
                }
                else
                {
        ...
        protected override void OnMouseDown(MouseEventArgs e)
        {
            ...
            Debug.WriteLine("Control is Focused ? " + Focused);
        ...
        public bool XmlSerialize(XmlNode xmlNode)
        {
            ...
            try
            {
                ...
                elementRoot = xmlDoc.CreateElement(ToolBox.Xml_Root_Node);
                elementRoot.SetAttribute(ToolBox.Xml_Version_Attr,"1");
                ...
                // Toolbox main config
                element1 = xmlDoc.CreateElement(ToolBox.Xml_Config_Node);
                ...
                // Tab height
                element1 = xmlDoc.CreateElement(ToolBox.Xml_TabHeight_Node);
                element1.AppendChild(xmlDoc.CreateTextNode(_tabHeight.ToString()));
                element0.AppendChild(element1);

                // Item height
                element1 = xmlDoc.CreateElement(ToolBox.Xml_ItemHeight_Node);
                element1.AppendChild(xmlDoc.CreateTextNode(_itemHeight.ToString()));
                element0.AppendChild(element1);

                // Tab spacing
                element1 = xmlDoc.CreateElement(ToolBox.Xml_TabSpacing_Node);
                element1.AppendChild(xmlDoc.CreateTextNode(_tabSpacing.ToString()));
                element0.AppendChild(element1);

                // Item spacing
                element1 = xmlDoc.CreateElement(ToolBox.Xml_ItemSpacing_Node);
                element1.AppendChild(xmlDoc.CreateTextNode(_itemSpacing.ToString()));
                element0.AppendChild(element1);

                // Layout delay
                element1 = xmlDoc.CreateElement(ToolBox.Xml_LayoutDelay_Node);
                element1.AppendChild(xmlDoc.CreateTextNode(_layoutDelay.ToString()));
                element0.AppendChild(element1);

                // Scroll delay
                element1 = xmlDoc.CreateElement(ToolBox.Xml_ScrollDelay_Node);
                element1.AppendChild(xmlDoc.CreateTextNode(_scrollDelay.ToString()));
                element0.AppendChild(element1);

                // Scroll wait
                element1 = xmlDoc.CreateElement(ToolBox.Xml_ScrollWait_Node);
                element1.AppendChild(xmlDoc.CreateTextNode(_scrollWait.ToString()));
                element0.AppendChild(element1);

                // Background color
                element1 = xmlDoc.CreateElement(ToolBox.Xml_BgColor_Node);
                ...
                element1 = xmlDoc.CreateElement(ToolBox.Xml_ItemBgColor_Node);
                ...
                // Item border color
                element1 = xmlDoc.CreateElement(ToolBox.Xml_ItemBorderColor_Node);
                ...
                element1 = xmlDoc.CreateElement(ToolBox.Xml_SelItemTxtColor_Node);
                ...
                element1 = xmlDoc.CreateElement(ToolBox.Xml_HoverItemTxtColor_Node);
                ...
                element1 = xmlDoc.CreateElement(ToolBox.Xml_NormTabTxtColor_Node);
                ...
                element1 = xmlDoc.CreateElement(ToolBox.Xml_SelTabTxtColor_Node);
                ...
                element1 = xmlDoc.CreateElement(ToolBox.Xml_HoverTabTxtColor_Node);
                ...
                element1 = xmlDoc.CreateElement(ToolBox.Xml_SelAllTxtInRename_Node);
                element1.AppendChild(xmlDoc.CreateTextNode(_selAllTextInRename.ToString().ToLower()));
                element0.AppendChild(element1);

                // "Show only one item per row" flag
                element1 = xmlDoc.CreateElement(ToolBox.Xml_ShowOneItemPerRow_Node);
                element1.AppendChild(xmlDoc.CreateTextNode(_onlyOneItemPerRow.ToString().ToLower()));
                element0.AppendChild(element1);

                // "Allow swapping by drag drop" flag
                element1 = xmlDoc.CreateElement(ToolBox.Xml_AllowDragSwap_Node);
                element1.AppendChild(xmlDoc.CreateTextNode(_allowDragSwap.ToString().ToLower()));
                element0.AppendChild(element1);

                // "use item color in rename" flag
                element1 = xmlDoc.CreateElement(ToolBox.Xml_UseItemClrInRen_Node);
                element1.AppendChild(xmlDoc.CreateTextNode(_useItemClrInRename.ToString().ToLower()));
                element0.AppendChild(element1);

                // Large item size
                element1 = xmlDoc.CreateElement(ToolBox.Xml_LargeItemSize_Node);
                element1.SetAttribute(ToolBox.Xml_Width_Attr ,_largeItemSize.Width.ToString() );
                element1.SetAttribute(ToolBox.Xml_Height_Attr,_largeItemSize.Height.ToString());
                element0.AppendChild(element1);

                // Small item size
                element1 = xmlDoc.CreateElement(ToolBox.Xml_SmallItemSize_Node);
                element1.SetAttribute(ToolBox.Xml_Width_Attr ,_smallItemSize.Width.ToString() );
                element1.SetAttribute(ToolBox.Xml_Height_Attr,_smallItemSize.Height.ToString());
                element0.AppendChild(element1);

                // Scroll buttons
                element1 = xmlDoc.CreateElement(ToolBox.Xml_Scrolls_Node);
                element0.AppendChild(element1);

                //Up Scroll
                itemElement = xmlDoc.CreateElement(_upScroll.ScrollDirection.ToString());
                itemElement.SetAttribute(ToolBox.Xml_Enabled_Attr,_upScroll.Enabled.ToString().ToLower());
                ...
                itemElement.SetAttribute(ToolBox.Xml_Enabled_Attr,_dnScroll.Enabled.ToString().ToLower());
                ...
                //Tabs
                element1 = xmlDoc.CreateElement(ToolBox.Xml_Tabs_Node);
                element0.AppendChild(element1);

                element1.SetAttribute(ToolBox.Xml_SelTabIdx_Attr,_toolBoxTabs.IndexOf(_selectedTab).ToString());
                ...
                for(iLoop1=0;iLoop1<_toolBoxTabs.Count;iLoop1++)
                {
                    ...
                    // Create the node
                    tabElement = SerializeToolBoxItem(tab,xmlDoc,ToolBox.Xml_Tab_Node);

                    //View Type
                    tabElement.SetAttribute(ToolBox.Xml_ViewMode_Attr,tab.View.ToString().ToLower());

                    // Item spacing
                    if(0 < tab.ItemSpacing)
                    {
                        tabElement.SetAttribute(ToolBox.Xml_ItemSpacing_Attr,tab.ItemSpacing.ToString());
                    }

                    // One item per row
                    if(tab.ShowOnlyOneItemPerRow)
                    {
                        tabElement.SetAttribute(ToolBox.Xml_OneItemPerRow_Attr,tab.ShowOnlyOneItemPerRow.ToString().ToLower());
                    }

                    // Item bg color
                    if(!tab.ItemBackgroundColor.IsEmpty)
                    {
                        tabElement.SetAttribute(ToolBox.Xml_ItemBgClr_Attr,ColorToHtmlString(tab.ItemBackgroundColor));
                    }

                    //Item border color
                    if(!tab.ItemBorderColor.IsEmpty)
                    {
                        tabElement.SetAttribute(ToolBox.Xml_ItemBorderClr_Attr,ColorToHtmlString(tab.ItemBorderColor));
                    }

                    //Item normal color
                    if(!tab.ItemNormalColor.IsEmpty)
                    {
                        tabElement.SetAttribute(ToolBox.Xml_ItemNormClr_Attr,ColorToHtmlString(tab.ItemNormalColor));
                    }

                    //Item selected color
                    if(!tab.ItemSelectedColor.IsEmpty)
                    {
                        tabElement.SetAttribute(ToolBox.Xml_ItemSelClr_Attr,ColorToHtmlString(tab.ItemSelectedColor));
                    }

                    //Item hover color
                    if(!tab.ItemHoverColor.IsEmpty)
                    {
                        tabElement.SetAttribute(ToolBox.Xml_ItemHoverClr_Attr,ColorToHtmlString(tab.ItemHoverColor));
                    }
                    ...
                    // Tab items
                    element0 = xmlDoc.CreateElement(ToolBox.Xml_TabItems_Node);
                    element0.SetAttribute(ToolBox.Xml_SelItemIdx_Attr,tab.SelectedItemIndex.ToString());
                    ...
                    {
                        item        = tab[iLoop2];
                        itemElement = SerializeToolBoxItem(item,xmlDoc, ToolBox.Xml_TabItem_Node);
        ...
        public bool XmlDeSerialize(XmlElement element)
        {
            ...
            try
            {
                ...
                if(element.Name != ToolBox.Xml_Root_Node)
                {
                    element = (XmlElement)element.SelectSingleNode(ToolBox.Xml_Root_Node);
                }

                // Config
                element0 = (XmlElement)element.SelectSingleNode(ToolBox.Xml_Config_Node);

                // Tab height
                element1    = (XmlElement)element0.SelectSingleNode(ToolBox.Xml_TabHeight_Node);
                _tabHeight  = SafeInt32FromString(element1.InnerText,18);

                // Item height
                element1    = (XmlElement)element0.SelectSingleNode(ToolBox.Xml_ItemHeight_Node);
                _itemHeight = SafeInt32FromString(element1.InnerText,20);

                // Tab spacing
                element1    = (XmlElement)element0.SelectSingleNode(ToolBox.Xml_TabSpacing_Node);
                _tabSpacing = SafeInt32FromString(element1.InnerText,1);

                // Item spacing
                element1     = (XmlElement)element0.SelectSingleNode(ToolBox.Xml_ItemSpacing_Node);
                _itemSpacing = SafeInt32FromString(element1.InnerText,1);

                // Layout delay
                element1     = (XmlElement)element0.SelectSingleNode(ToolBox.Xml_LayoutDelay_Node);
                _layoutDelay = SafeInt32FromString(element1.InnerText,10);

                // Scroll delay
                element1     = (XmlElement)element0.SelectSingleNode(ToolBox.Xml_ScrollDelay_Node);
                _scrollDelay = SafeInt32FromString(element1.InnerText,60);

                // Scroll wait
                element1     = (XmlElement)element0.SelectSingleNode(ToolBox.Xml_ScrollWait_Node);
                _scrollWait  = SafeInt32FromString(element1.InnerText,500);

                // Background color
                element1     = (XmlElement)element0.SelectSingleNode(ToolBox.Xml_BgColor_Node);
                BackColor    = ColorFromHtmlString(element1.InnerText,BackColor);

                // Item background color
                element1     = (XmlElement)element0.SelectSingleNode(ToolBox.Xml_ItemBgColor_Node);
                _itemBgColor = ColorFromHtmlString(element1.InnerText,Color.Empty);

                // Item border color
                element1     = (XmlElement)element0.SelectSingleNode(ToolBox.Xml_ItemBorderColor_Node);
                _itemBorderColor = ColorFromHtmlString(element1.InnerText,Color.Empty);

                // Normal item color
                element1     = (XmlElement)element0.SelectSingleNode(ToolBox.Xml_NormItemColor_Node);
                _itemNormalColor = ColorFromHtmlString(element1.InnerText,SystemColors.Control);

                // Selected item color
                element1     = (XmlElement)element0.SelectSingleNode(ToolBox.Xml_SelItemColor_Node);
                _selectedItemColor = ColorFromHtmlString(element1.InnerText,Color.White);

                // Hover item color
                element1     = (XmlElement)element0.SelectSingleNode(ToolBox.Xml_HoverItemColor_Node);
                _itemHoverColor = ColorFromHtmlString(element1.InnerText,SystemColors.Control);

                // Normal item text color
                element1     = (XmlElement)element0.SelectSingleNode(ToolBox.Xml_NormItemTxtColor_Node);
                _itemTxtNormalColor = ColorFromHtmlString(element1.InnerText,Color.Empty);

                // Selected item text color
                element1     = (XmlElement)element0.SelectSingleNode(ToolBox.Xml_SelItemTxtColor_Node);
                _itemTxtSelectColor = ColorFromHtmlString(element1.InnerText,Color.Empty);

                // Hover item text color
                element1     = (XmlElement)element0.SelectSingleNode(ToolBox.Xml_HoverItemTxtColor_Node);
                _itemTxtHoverColor = ColorFromHtmlString(element1.InnerText,Color.Empty);

                // Normal tab text color
                element1     = (XmlElement)element0.SelectSingleNode(ToolBox.Xml_NormTabTxtColor_Node);
                _tabTxtNormalColor = ColorFromHtmlString(element1.InnerText,Color.Empty);

                // Selected tab text color
                element1     = (XmlElement)element0.SelectSingleNode(ToolBox.Xml_SelTabTxtColor_Node);
                _tabTxtSelectColor = ColorFromHtmlString(element1.InnerText,Color.Empty);

                // Hover tab text color
                element1     = (XmlElement)element0.SelectSingleNode(ToolBox.Xml_HoverTabTxtColor_Node);
                _tabTxtHoverColor = ColorFromHtmlString(element1.InnerText,Color.Empty);

                // "Show only one item per row" flag
                element1     = (XmlElement)element0.SelectSingleNode(ToolBox.Xml_ShowOneItemPerRow_Node);
                _onlyOneItemPerRow = SafeBoolFromString(element1.InnerText,false);

                // "Select All text while renaming" flag
                element1     = (XmlElement)element0.SelectSingleNode(ToolBox.Xml_SelAllTxtInRename_Node);
                _selAllTextInRename = SafeBoolFromString(element1.InnerText,true);

                // "Allow swapping by drag drop" flag
                element1     = (XmlElement)element0.SelectSingleNode(ToolBox.Xml_AllowDragSwap_Node);
                _allowDragSwap  = SafeBoolFromString(element1.InnerText,true);

                // "use item color in rename" flag
                element1     = (XmlElement)element0.SelectSingleNode(ToolBox.Xml_UseItemClrInRen_Node);
                _useItemClrInRename = SafeBoolFromString(element1.InnerText,true);

                // Large item size
                element1     = (XmlElement)element0.SelectSingleNode(ToolBox.Xml_LargeItemSize_Node);
                _largeItemSize.Width  = SafeInt32FromString(element1.GetAttribute(ToolBox.Xml_Width_Attr) ,64);
                _largeItemSize.Height = SafeInt32FromString(element1.GetAttribute(ToolBox.Xml_Height_Attr),64);

                // Small item size
                element1     = (XmlElement)element0.SelectSingleNode(ToolBox.Xml_SmallItemSize_Node);
                _smallItemSize.Width  = SafeInt32FromString(element1.GetAttribute(ToolBox.Xml_Width_Attr) ,32);
                _smallItemSize.Height = SafeInt32FromString(element1.GetAttribute(ToolBox.Xml_Height_Attr),32);

                // Scroll buttons
                element1     = (XmlElement)element0.SelectSingleNode(ToolBox.Xml_Scrolls_Node);

                //Up Scroll
                itemElement       = (XmlElement)element1.SelectSingleNode(ScrollDirection.Up.ToString());
                _upScroll.Enabled = SafeBoolFromString(itemElement.GetAttribute(ToolBox.Xml_Enabled_Attr),true);
                element2          = (XmlElement)itemElement.SelectSingleNode(ToolBox.Xml_ToolTip_Node);
                tempString        = element2.InnerText;
                ...
                _dnScroll.Enabled = SafeBoolFromString(itemElement.GetAttribute(ToolBox.Xml_Enabled_Attr),true);
                element2          = (XmlElement)itemElement.SelectSingleNode(ToolBox.Xml_ToolTip_Node);
                tempString        = element2.InnerText;
                ...
                // Tabs
                element1     = (XmlElement)element0.SelectSingleNode(ToolBox.Xml_Tabs_Node);
                selTabIdx    = SafeInt32FromString(element1.GetAttribute(ToolBox.Xml_SelTabIdx_Attr),-1);

                tabNodes     = element1.SelectNodes(ToolBox.Xml_Tab_Node);
                ...
                for(iLoop1=0;iLoop1<tabNodes.Count;iLoop1++)
                {
                    ...
                    try
                    {
                        tab.View = (ViewMode)Enum.Parse(typeof(ViewMode),tabElement.GetAttribute(ToolBox.Xml_ViewMode_Attr),true);
                    }
                    catch
                    {
                    ...
                    // Item spacing
                    tab.ItemSpacing           = SafeInt32FromString(tabElement.GetAttribute(ToolBox.Xml_ItemSpacing_Attr),0);

                    // One item per row
                    tab.ShowOnlyOneItemPerRow = SafeBoolFromString(tabElement.GetAttribute(ToolBox.Xml_OneItemPerRow_Attr),false);

                    // Item bg color
                    tab.ItemBackgroundColor   = ColorFromHtmlString(tabElement.GetAttribute(ToolBox.Xml_ItemBgClr_Attr),Color.Empty);

                    // Item border color
                    tab.ItemBorderColor       = ColorFromHtmlString(tabElement.GetAttribute(ToolBox.Xml_ItemBorderClr_Attr),Color.Empty);

                    //Item normal color
                    tab.ItemNormalColor       = ColorFromHtmlString(tabElement.GetAttribute(ToolBox.Xml_ItemNormClr_Attr),Color.Empty);

                    //Item selected color
                    tab.ItemSelectedColor     = ColorFromHtmlString(tabElement.GetAttribute(ToolBox.Xml_ItemSelClr_Attr),Color.Empty);

                    //Item hover color
                    tab.ItemHoverColor        = ColorFromHtmlString(tabElement.GetAttribute(ToolBox.Xml_ItemHoverClr_Attr),Color.Empty);
                    ...
                    element1   = (XmlElement)tabElement.SelectSingleNode(ToolBox.Xml_TabItems_Node);
                    itemNodes  = element1.SelectNodes(ToolBox.Xml_TabItem_Node);
                    selItemIdx = SafeInt32FromString(element1.GetAttribute(ToolBox.Xml_SelItemIdx_Attr),0);
